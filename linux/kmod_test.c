...
int rohc_couple_init_phase1(struct rohc_couple *couple, int index)
{
	...
	rohc_info("\tUncompressed, RTP, UDP, ESP, IP and TCP profiles enabled for ROHC compressor successfully set\n");
...
int rohc_couple_init_phase2(struct rohc_couple *couple, int index)
{
	...
	rohc_info("\tUncompressed, RTP, UDP, ESP, IP and TCP profiles enabled for ROHC decompressor successfully set\n");
			     ...
			     loff_t *ppos)
{
	...
	if (couple->ip_size_current_in == couple->ip_size_total_in) {
		 ...
		 * the ROHC packet
		 */
		rohc_info("copy %zu bytes of feedback data before ROHC packet\n",
			  couple->feedback_to_send.len);
		if (couple->feedback_to_send.len >
		    rohc_buf_avail_len(rohc_packet)) {
			rohc_err("ROHC buffer is too small for %zu bytes of feedback data\n",
				 couple->feedback_to_send.len);
			goto error;
		}
			       ...
			       loff_t *ppos)
{
	...
	if (couple->rohc_size_curr_in == couple->rohc_size_total_in) {
		 ...
		 * in consequence
		 */
		rohc_info("deliver %zu-byte received feedback to compressor\n",
			  rcvd_feedback.len);
		if (!rohc_comp_deliver_feedback2(comp_associated,
						 rcvd_feedback)) {
			rohc_err("failed to deliver received feedback to comp.\n");
			goto error;
		}
		kfree(couple->rohc_pkt_in);
		couple->rohc_pkt_in = NULL;
