...
int main(int argc, char *argv[])
{
	...
	/* rohc_decompress3() */
	{
		...
		CHECK(rohc_decompress3(NULL, pkt1, &pkt2, NULL, NULL) == ROHC_ERROR);
		CHECK(pkt2.len == 0);
		pkt1.len = 0;
		CHECK(rohc_decompress3(decomp, pkt1, &pkt2, NULL, NULL) == ROHC_ERROR);
		CHECK(pkt2.len == 0);
		pkt1.len = 1;
		CHECK(rohc_decompress3(decomp, pkt1, NULL, NULL, NULL) == ROHC_ERROR);
		CHECK(pkt2.len == 0);
		for(size_t i = 0; i < (pkt.len - 2); i++)
		{
			...
			pkt2.offset = 0;
			pkt2.len = 0;
			CHECK(rohc_decompress3(decomp, pkt, &pkt2, NULL, NULL) == ROHC_ERROR);
			CHECK(pkt2.len == 0);
		}
		pkt2.max_len = pkt.len - 2;
		pkt2.offset = 0;
		pkt2.len = 0;
		CHECK(rohc_decompress3(decomp, pkt, &pkt2, NULL, NULL) == ROHC_OK);
		CHECK(pkt2.len > 0);
	}
