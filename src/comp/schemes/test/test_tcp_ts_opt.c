...
static void test_tcp_ts_opt(void **state __attribute__((unused)))
{
	...
	is_ok = c_tcp_ts_lsb_code(&context, ts, &wlsb, rohc_data, 0, &rohc_len);
	assert_true(is_ok == false);
	is_ok = c_tcp_ts_lsb_code(&context, ts, &wlsb, rohc_data, 1, &rohc_len);
	...
	is_ok = c_tcp_ts_lsb_code(&context, ts, &wlsb, rohc_data, 1, &rohc_len);
	assert_true(is_ok == false);
	is_ok = c_tcp_ts_lsb_code(&context, ts, &wlsb, rohc_data, 2, &rohc_len);
	assert_true(is_ok == true);
	...
	is_ok = c_tcp_ts_lsb_code(&context, ts, &wlsb, rohc_data, 2, &rohc_len);
	assert_true(is_ok == false);
	is_ok = c_tcp_ts_lsb_code(&context, ts, &wlsb, rohc_data, 3, &rohc_len);
	assert_true(is_ok == true);
	...
	is_ok = c_tcp_ts_lsb_code(&context, ts, &wlsb, rohc_data, 3, &rohc_len);
	assert_true(is_ok == false);
	is_ok = c_tcp_ts_lsb_code(&context, ts, &wlsb, rohc_data, 4, &rohc_len);
	assert_true(is_ok == true);
