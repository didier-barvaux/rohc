...
static rohc_packet_t c_rtp_decide_SO_packet(const struct rohc_comp_ctxt *const context)
{
	        ...
	        !rfc3095_ctxt->tmp.sn_14bits_possible)
	{
		...
		rohc_comp_debug(context, "choose packet IR-DYN because SN cannot be "
	        ...
	        is_ts_scaled &&
	        (is_ts_deducible || rfc3095_ctxt->tmp.nr_ts_bits == 0) &&
	        ...
	        !is_ext3_required)
	{
		...
		rohc_comp_debug(context, "choose packet UO-0 because less than 4 SN bits "
	        ...
	        rfc3095_ctxt->tmp.sn_4bits_possible &&
	        nr_ipv4_non_rnd == 0 &&
	        is_ts_scaled && rfc3095_ctxt->tmp.nr_ts_bits <= 6 &&
	        !rfc3095_ctxt->tmp.rtp_padding_changed &&
	        !rfc3095_ctxt->tmp.rtp_ext_changed &&
	        !rfc3095_ctxt->tmp.rtp_pt_changed &&
	        !is_ext3_required)
	{
		...
		rohc_comp_debug(context, "choose packet UO-1-RTP because neither of "
		                ...
		                nr_of_ip_hdr, rfc3095_ctxt->tmp.nr_ts_bits, nr_of_ip_hdr);
	}
	else if(!rnd_changed &&
	        rfc3095_ctxt->tmp.sn_4bits_possible &&
	        ...
	        !is_ext3_required)
	{
		...
		rohc_comp_debug(context, "choose packet UO-1-ID because only one of the "
	        ...
	        rfc3095_ctxt->tmp.sn_4bits_possible &&
	        nr_ipv4_non_rnd_with_bits == 0 &&
	        is_ts_scaled && rfc3095_ctxt->tmp.nr_ts_bits <= 5 &&
	        !rfc3095_ctxt->tmp.rtp_padding_changed &&
	        !rfc3095_ctxt->tmp.rtp_ext_changed &&
	        !rfc3095_ctxt->tmp.rtp_pt_changed &&
	        !is_ext3_required)
	{
		...
		rohc_comp_debug(context, "choose packet UO-1-TS because neither of the "
	        ...
	        sdvl_can_length_be_encoded(rfc3095_ctxt->tmp.nr_ts_bits))
	{
                                           ...
                                           const size_t nr_ipv4_non_rnd_with_bits)
{
	        ...
	        sdvl_can_length_be_encoded(rfc3095_ctxt->tmp.nr_ts_bits))
	{
		...
		rohc_comp_debug(ctxt, "choose packet UOR-2-ID because at least "
	                  ...
	                  rfc3095_ctxt->ip_hdr_nr, rfc3095_ctxt->tmp.nr_ts_bits);
	}
	else
	{
