...
int main(int argc, char *argv[])
{
	...
	/* rohc_comp_reserve_ctxt() */
	{
		...
		struct rohc_buf pkt = rohc_buf_init_full(buf, sizeof(buf), ts);
		const rohc_cid_t cid_to_reserve = 3;

		CHECK(rohc_comp_reserve_ctxt(NULL, cid_to_reserve, pkt) == false);
		CHECK(rohc_comp_reserve_ctxt(comp, ROHC_SMALL_CID_MAX + 1, pkt) == false);
		CHECK(rohc_comp_reserve_ctxt(comp, cid_to_reserve, pkt) == true);
		CHECK(rohc_comp_unreserve_ctxt(NULL, cid_to_reserve) == false);
		CHECK(rohc_comp_unreserve_ctxt(comp, ROHC_SMALL_CID_MAX + 1) == false);
		CHECK(rohc_comp_unreserve_ctxt(comp, 1) == false);
		CHECK(rohc_comp_unreserve_ctxt(comp, 2) == false);
		CHECK(rohc_comp_unreserve_ctxt(comp, cid_to_reserve) == true);
	}
