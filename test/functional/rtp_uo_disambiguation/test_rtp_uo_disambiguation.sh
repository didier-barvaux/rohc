...
#
# file:        test_rtp_uo_disambiguation.sh
# description: Check that the ROHC library correctly disambiguates UO-1* / UOR-2* packets
# author:      Didier Barvaux <didier@barvaux.org>
#
# This script may be used by creating a link "test_rtp_uo_disambiguation_CAPTURE.sh"
# where:
#    CAPTURE  is the source capture to use for the test, it must ends with
#             the name of the expected packet (ie. uo1rtp, uo1id, uo1ts,
#             uor2rtp, uor2id or uor2ts)
#
# Script arguments:
#    test_rtp_uo_disambiguation_CAPTURE.sh [verbose [verbose]]
# where:
...
if [ "x$MAKELEVEL" != "x" ] ; then
	BASEDIR="${srcdir}"
	APP="./test_rtp_uo_disambiguation${CROSS_COMPILATION_EXEEXT}"
else
	BASEDIR=$( dirname "${SCRIPT}" )
	APP="${BASEDIR}/test_rtp_uo_disambiguation${CROSS_COMPILATION_EXEEXT}"
fi

# extract the source capture and the expected packet from the name of the script
CAPTURE_NAME=$( echo "${SCRIPT}" | \
                ${SED} -e 's#^.*/test_rtp_uo_disambiguation_##' -e 's#\.sh$##' )
