...
if len(sys.argv) != 4:
    print 'usage: generate_reference_stream_rtp.py <pkts_nr> <streams_nr> <pkt_len>'
    sys.exit(1)

pkts_nr = int(sys.argv[1])
streams_nr = int(sys.argv[2])
pkt_len = int(sys.argv[3])
pcap_file_name = "reference_stream_rtp_%ipkts_%istreams_%iBpkt.pcap" \
        % (pkts_nr, streams_nr, pkt_len)

print "generate %i RTP stream(s) with %i packets in file '%s'" \
