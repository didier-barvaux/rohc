...
#
#
# file:        generate_reference_stream_ipv6_rtp.py
...
if len(sys.argv) != 4:
    print 'usage: generate_reference_stream_ipv6_rtp.py <pkts_nr> <streams_nr> <pkt_len>'
    sys.exit(1)

pkts_nr = int(sys.argv[1])
streams_nr = int(sys.argv[2])
pkt_len = int(sys.argv[3])
pcap_file_name = "reference_stream_ipv6_rtp_%ipkts_%istreams_%iBpkt.pcap" \
...
for num in range(0, pkts_nr):
    ...
    packet = Ether(src='00:00:00:00:00:01', dst='00:00:00:00:00:02')/Dot1Q(vlan=1)/IPv6(src='fe80::1:1', dst='fe80::0:%i' % ((stream_num / 100) % 254))/UDP(sport=1234+stream_num, dport=1234, chksum=0)/RTP(sequence=streams[stream_num]['sn'], timestamp=streams[stream_num]['ts'])/payload[:payload_len]
